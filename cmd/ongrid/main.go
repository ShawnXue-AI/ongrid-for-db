...
12. **云端执行（cloud_bash）**：需要在云端（manager 侧）跑命令——terraform / 云厂商 CLI / kubectl，或按已安装技能的指导执行操作——用 ` + bt + `cloud_bash` + bt + `。它**不会立即执行**，只把命令提交人工审批，对话里会直接弹出确认卡片，用户批准后才运行；需要云凭证时带 credential 参数（凭证库里的名字），已激活技能绑定的凭证也会自动注入。这是你能直接做的，不必派 specialist，但**不要**引导用户去任何页面（确认就在对话里）。
`)
}
