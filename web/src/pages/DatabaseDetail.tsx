...
const DB_METRICS: Record<string, MetricDef[]> = {
  ...
  redis: [
    ...
    { label: 'CPU 使用率', expr: 'sum(rate(redis_cpu_sys_seconds_total{db_type="{{db_type}}"}[5m]) + rate(redis_cpu_user_seconds_total{db_type="{{db_type}}"}[5m]))', unit: 'cpu/s', color: '#f97316' },
    { label: '网络接收', expr: 'sum(rate(redis_net_input_bytes_total{db_type="{{db_type}}"}[5m]))', unit: 'B/s', color: '#06b6d4' },
    { label: '网络发送', expr: 'sum(rate(redis_net_output_bytes_total{db_type="{{db_type}}"}[5m]))', unit: 'B/s', color: '#14b8a6' },
    { label: '逐出键', expr: 'sum(increase(redis_evicted_keys_total{db_type="{{db_type}}"}[15m]))', unit: '15m', color: '#ef4444' },
  ],
  mongodb: [
    ...
    { label: '网络接收', expr: 'sum(rate(mongodb_ss_network_bytes_in{db_type="{{db_type}}"}[5m]))', unit: 'B/s', color: '#06b6d4' },
    { label: '网络发送', expr: 'sum(rate(mongodb_ss_network_bytes_out{db_type="{{db_type}}"}[5m]))', unit: 'B/s', color: '#14b8a6' },
    { label: '打开游标', expr: 'mongodb_ss_cursors_total_open{db_type="{{db_type}}"}', unit: 'cursors', color: '#a78bfa' },
    { label: '写入QPS', expr: 'sum(rate(mongodb_ss_opcounters_insert{db_type="{{db_type}}"}[5m]))', unit: 'w/s', color: '#f97316' },
  ],
  oracle: [
